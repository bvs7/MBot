...
class MState:
    ...
    # TODO: Fix saves etc
    def __init__(self, playerIDs, mainComm, mafiaComm,
    ...
    def startGame(self, nextPlayerIDs, preferences=None):
        ...
        for player in self.players:
            self.mainComm.add(player.id, self.lobby.getName(player_id))
            self.mainComm.send(ROLE_EXPLAIN[player.role],player.id)
            if player.role in MAFIA_ROLES:
                self.mafiaComm.add(player.id, self.lobby.getName(player_id))
